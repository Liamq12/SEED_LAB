...
def serialHandler(serialConn):
    ...
    while True:
        ...
        if(serialConn.poll()):
            ...
            if(messageObj != f"0" and delay2-delayTurn>0.9 ):
                if(messageObj == f"1"):
                    serialConn.close()
                    break
                elif(messageObj ==f"SL\n"):
                    amountSleep= .04-(delay2-delay)
                    if (amountSleep > 0):
                        sleep(amountSleep)
...
if __name__ == '__main__':
    ...
    prevTime = time.time()
    turnTo=2
    upperGreen = np.array([110, 200, 180])
    lowerGreen = np.array([45,80,65])
